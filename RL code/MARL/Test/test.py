...
os.makedirs("models", exist_ok=True) # Creates the directory if it's not already there
env = simple_tag_v3.parallel_env(num_good=1, num_adversaries=1, render_mode=None)
env.reset(seed=42)

# alpha = 0.01
alpha = 0.001 
WARMUP_STEPS = 10000 # Let the buffer fill with 10k random samples first
gamma = 0.95
epsilon = 1.0
epsilon_min = 0.05
...
for episode in progress_bar:
    ...
    while not done:
        ...
        if len(agent_buffer) >= BATCH_SIZE and global_total_steps > WARMUP_STEPS:
        ...
        if len(adversary_buffer) >= BATCH_SIZE and global_total_steps > WARMUP_STEPS:
            # Sample a batch
